...
void main() async {
  ...
  Socket sock = await Socket.connect('192.168.1.193', 80);
  runApp(MyApp(sock));
}
...
class _MyHomePageState extends State<MyHomePage> {
  ...
  void interpolateHelper(Map position1, Map position2, int resolution, int positionIndex){
  ...
  void interpolateAll(int resolution){
    ...
    for (int i = 0; i < positionsList.length-1; i++){
      print (i);
      print(interpolateSegmentDone);
      if (interpolateSegmentDone == true){
        interpolateHelper(positionsList[i], positionsList[i+1], resolution, i);
      }
      else{
        --i;
      }
